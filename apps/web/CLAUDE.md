...
- **Build command**:
  ```
  rm -rf node_modules apps/*/node_modules packages/*/node_modules && pnpm install --no-frozen-lockfile --store-dir /tmp/pnpm-store && cd apps/web && ./node_modules/.bin/vp build
  ```
  The cause of the long "Cannot find native binding `@voidzero-dev/vite-plus-linux-x64-gnu`" saga: Cloudflare restores a **cached pnpm store** ("Restoring from dependencies cache") from the earliest builds, which is missing the Linux native bindings (vite-plus, rolldown, oxc, lightningcss). Reusing that store never re-fetches them, and clearing `node_modules` doesn't help because the store cache is separate. Verified in Docker that a clean Linux install (glibc and musl) fetches the bindings and vite-plus loads fine — so the lockfile/pnpm/config are correct; only Cloudflare's cache is poisoned. Pointing pnpm at a fresh `--store-dir /tmp/pnpm-store` (not covered by Cloudflare's cache) forces a clean fetch of all packages including the bindings. Alternative: clear the build cache in the Cloudflare dashboard and use the simpler `cd apps/web && ./node_modules/.bin/vp build`. There is no standalone `vite` binary, so the build runs through the local `vp`. Without a build, `dist/` is empty and `wrangler deploy` fails with `entry-point @tanstack/react-start/server-entry not found`.
