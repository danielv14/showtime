...
// `DetailView` renders `SeasonEpisodes`, whose `EpisodeDetailPanel` imports a
// server function from `#/server/media`; that module reaches into
// `cloudflare:workers` (via the cache), which can't load under jsdom. These
// tests render movie details and never exercise the episode fetch, so stub the
// server module's runtime surface.
vi.mock("#/server/media", () => ({ getEpisodeDetail: vi.fn() }));
