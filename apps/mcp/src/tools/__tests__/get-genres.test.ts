...
import { createFakeClients } from "./fake-clients.js";
...
describe("get_genres tool", () => {
  it("returns both movie and TV genre lists by default", async () => {
    ...
    const clients = createFakeClients({ tmdb: { getMovieGenres, getTvGenres } });

    const result = (await getGenresTool.handler({}, clients)) as Record<string, unknown>;
  ...
  it("returns only movie genres when mediaType is 'movie'", async () => {
    ...
    const result = (await getGenresTool.handler({ mediaType: "movie" }, clients)) as Record<
      string,
      unknown
    >;
  ...
  it("returns only TV genres when mediaType is 'tv'", async () => {
    ...
    const result = (await getGenresTool.handler({ mediaType: "tv" }, clients)) as Record<
      string,
      unknown
    >;
  ...
  it("exposes genres as name + id pairs", async () => {
    ...
    const result = (await getGenresTool.handler({ mediaType: "movie" }, clients)) as {
